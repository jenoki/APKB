...
void USBCheckHIDRequest(void)
{
    ...
    if(SetupPkt.bIntfID != HID_INTF_IDK && SetupPkt.bIntfID != HID_INTF_IDM)
            return;
    ...
    if(SetupPkt.bRequest == GET_DSC)
    {
        switch(SetupPkt.bDescriptorType)
        {
            ...
            case DSC_RPT:
                if(USBActiveConfiguration == 1)
                {
                    if (SetupPkt.bIntfID == HID_INTF_IDK)
                    {
                        USBEP0SendROMPtr((ROM BYTE*)&hid_rptk,
                                sizeof(hid_rptk),//See usbcfg.h
                                USB_EP0_INCLUDE_ZERO);
                    }else if(SetupPkt.bIntfID == HID_INTF_IDM)
                    {
                        USBEP0SendROMPtr((ROM BYTE*)&hid_rptm,
                                sizeof(hid_rptm),//See usbcfg.h
                                USB_EP0_INCLUDE_ZERO);
                    }
                }
                break;
